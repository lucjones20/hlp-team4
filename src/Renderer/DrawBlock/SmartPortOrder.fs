...
let reOrderPorts 
    ...
    let sortedMapOfStaticPorts = SmartHelpers.sortPorts portsStaticByEdge optimalOrder // Sorts non changing ports that are connected so the optimal order for the connected ports is found
    let sortedListOfStaticPorts = SmartHelpers.getListOfPortsFromMap sortedMapOfStaticPorts // list in order required for the static ports - makes it easier to sort the connectingPortsIds
